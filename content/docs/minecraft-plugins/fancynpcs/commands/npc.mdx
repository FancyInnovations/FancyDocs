...
Note: The name check is strict and only allows `A-Z a-z 0-9 _ - /` characters.
...
Note: Empty message placeholder is `@none`
...
Note: Re-connecting to the server might be required for changes to take effect.
...
- `fancynpcs.command.npc.action.add.(actionType | *)` for the specific action type
...
Note: Formerly known as `/npc interactionCooldown`.
